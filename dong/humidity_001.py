...
with open('format/humidity_json_format.json') as json_file:
    msg = json_file.read()
    device_code = '001'
...
while True : 
    ...
    external_value = value.call_external_humidity()
    tmp = int(external_value) / 2
    inner_value = str(tmp)
    
    m_p.mqtt_publish(msg % (device_code, time_num, lan, conf.broker_ip, external_value, inner_value))
    time.sleep(conf.call_delay('humidity'))
